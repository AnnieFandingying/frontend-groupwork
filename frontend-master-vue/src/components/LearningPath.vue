    ...
    <transition name="welcome-fade">
      <div v-if="showWelcome" class="welcome-screen">
        <div class="welcome-content">
          <!-- 转圈动画 -->
          <div class="welcome-dots-spinner">
            <div class="dot" v-for="i in 12" :key="i" :style="{ '--i': i }"></div>
          </div>
    ...
    <div class="path-header" ref="headerRef">
      <div class="header-content">
        ...
        <div class="header-text">
          <h1 class="header-title">ai个性化学习路线生成</h1>
        ...
        <div class="ai-main">
          <div class="hero-card">
            <div class="hero-pills">
              ...
              <span class="pill pill-light">{{ overviewTitle }}</span>
            </div>
            <h1 class="hero-title">{{ overviewTitle }}</h1>
          ...
          <div class="path-card-grid">
            ...
            >
              ...
              <div class="card-footer">
                ...
                <span class="card-link" @click.stop="activeCardId = card.id">查看详情 →</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="insight-card radar-preview-card" @click="showRadarModal = true">
          <div class="insight-header">
          <div>
          <p class="insight-label">技能雷达</p>
          <h3 class="insight-title">掌握度概览</h3>
          </div>
          <span class="mini-pill">点击展开</span>
          </div>
          ...
          fill-opacity="0.25"
          stroke="url(#radarStrokeMini)"
          stroke-width="3"
          />
          <defs>
          <linearGradient id="radarGradientMini" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" style="stop-color:#3b82f6;stop-opacity:0.8" />
          <stop offset="100%" style="stop-color:#8b5cf6;stop-opacity:0.6" />
          </linearGradient>
          <linearGradient id="radarStrokeMini" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <div class="insight-card monthly-preview-card" v-if="monthlyPreviewBars.length > 0" @click="showMonthlyModal = true">
          <div class="insight-header">
          <div>
          <p class="insight-label">月度计划</p>
          <h3 class="insight-title">节奏预览</h3>
          </div>
          <button type="button" class="mini-pill mini-pill-button" @click.stop="showMonthlyModal = true">点击展开</button>
          </div>
          ...
          <div class="insight-card progress-card">
            <div class="insight-header">
              <div>
                <p class="insight-label">学习进度</p>
                <h3 class="insight-title">完成情况</h3>
              </div>
            </div>
...
<div class="stages-timeline-section" v-show="activeCardId === 'overview'">
        <div class="section-header">
          <h2 class="section-title">
            <span class="section-icon icon-cluster">
        ...
        <div class="stages-horizontal-timeline">
          <div v-for="(stage, index) in learningPath.phases" :key="stage.id" class="stage-item">
            ...
            <div class="stage-content-box" :class="`stage-card--${index % 4}`">
                  ...
                  <div v-for="topic in phase.topics" :key="topic.id" class="topic-card">
                    <div class="topic-header">
                      <input 
                        ...
                        @change="handleTopicToggle(phase, topic)"
              ...
              <div class="timeline-card">
                ...
                <div class="timeline-tasks">
                  ...
                  >
                    <input 
                      type="checkbox" 
                      :id="'timeline-' + topic.id" 
                      v-model="topic.completed"
                      @change="updateTimelineProgress(phase)"
                      class="timeline-checkbox"
                    />
                    <label :for="'timeline-' + topic.id" class="timeline-task-label">
                      {{ topic.name }}
                    </label>
                  </div>
                </div>
      ...
      <!-- 雷达图悬浮窗口 -->
      <div v-if="showRadarModal" class="modal-overlay" @click.self="showRadarModal = false">
        <div class="modal-card">
          <div class="modal-header">
            <div>
              <p class="modal-eyebrow">&#x6280;&#x80FD;&#x96F7;&#x8FBE;</p>
              <h3 class="modal-title">&#x5B8C;&#x6574;&#x8BC4;&#x4F30;&#x5206;&#x6570;</h3>
            </div>
            <button class="modal-close" type="button" @click="showRadarModal = false">
          ...
          <div class="modal-body">
            <div class="radar-modal-grid">
              <div class="radar-modal-chart">
                <svg :width="radarSize" :height="radarSize" class="radar-svg">
                  <g class="radar-grid">
                    <circle 
                      ...
                      :stroke-width="level === 5 ? 2 : 1"
                    />
                    <g v-for="(skill, index) in radarSkills" :key="skill.name + '-modal'">
                      <line 
                        :x1="radarSize/2" 
                        :y1="radarSize/2"
                  ...
                  <polygon 
                    ...
                    fill="url(#radarGradientModal)"
                    fill-opacity="0.3"
                    stroke="url(#radarStrokeModal)"
                    stroke-width="3"
                  />
                  <g v-for="(point, index) in radarDataPoints" :key="index + '-modal'">
                    <circle 
                      :cx="point.x" 
                      :cy="point.y" 
                      r="5"
                  ...
                  <g v-for="(skill, index) in radarSkills" :key="skill.name + '-label-modal'">
                    <text 
                  ...
                  <defs>
                    <linearGradient id="radarGradientModal" x1="0%" y1="0%" x2="100%" y2="100%">
                      <stop offset="0%" style="stop-color:#3b82f6;stop-opacity:0.8" />
                      <stop offset="100%" style="stop-color:#8b5cf6;stop-opacity:0.6" />
                    </linearGradient>
                    <linearGradient id="radarStrokeModal" x1="0%" y1="0%" x2="100%" y2="100%">
                      <stop offset="0%" style="stop-color:#3b82f6" />
                      <stop offset="100%" style="stop-color:#8b5cf6" />
                    </linearGradient>
                  </defs>
              ...
              <div class="radar-modal-list">
                <div v-for="skill in radarSkills" :key="skill.name + '-score'" class="radar-score-row">
                  <span class="radar-score-name">{{ skill.name }}</span>
                  <div class="radar-score-bar">
                    <span class="radar-score-fill" :style="{ width: skill.score * 10 + '%', backgroundColor: skill.color }"></span>
                  </div>
                  <span class="radar-score-value">{{ skill.score }}/10</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="showMonthlyModal" class="modal-overlay" @click.self="showMonthlyModal = false">
        <div class="modal-card modal-card-wide">
          <div class="modal-header">
            <div>
              <p class="modal-eyebrow">&#x6708;&#x5EA6;&#x8BA1;&#x5212;</p>
              <h3 class="modal-title">&#x5B8C;&#x6574;&#x7518;&#x7279;&#x56FE;</h3>
            </div>
            <button class="modal-close" type="button" @click="showMonthlyModal = false">
              <svg class="modal-close-icon" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M6 6L18 18" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/>
                <path d="M18 6L6 18" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/>
              </svg>
          ...
          <div class="modal-body">
            <div class="gantt-section" v-if="learningPath.monthlyPlan && learningPath.monthlyPlan.length > 0">
              <div class="section-header">
                <h2 class="section-title">
                  <span class="section-icon">&#x1F4C5;</span>
                  &#x6708;&#x5EA6;&#x5B66;&#x4E60;&#x8BA1;&#x5212;
                  ...
                  <div v-for="task in flattenedTasks" :key="task.id" class="gantt-row">
                    <div class="gantt-task-name">
                      <input 
                        ...
                        v-model="task.source.completed"
                        @change="syncMonthlyTaskCompletion(task.source)"
                        class="task-checkbox"
                      />
                      <label :for="'modal-task-' + task.id">{{ task.name }}</label>
                    </div>
                    <div class="gantt-timeline">
                      <div 
                        ...
                        :style="{
                          ...
                          backgroundColor: task.source.completed ? '#10b981' : getTaskColor(task.weekNumber)
                        }"
                        :class="{ 'completed': task.source.completed }"
                        :title="getGanttTitle(task)"
                      >
      ...
      </div><!-- 结束 card-detail-stack -->
    </div><!-- 结束 learning-path-section -->

    </div><!-- 结束 scrollable-content -->

    <!-- 加载动画覆盖层 -->
    <div v-if="isGenerating" class="loading-overlay">
      <div class="loading-content loading-cup-scene">
        <div class="cup-card">
          <div class="cup-illustration">
            <div class="cup">
              ...
              <div class="cup-wave"></div>
              <div class="cup-splash">
                <span class="splash s1"></span>
                <span class="splash s2"></span>
                <span class="splash s3"></span>
                <span class="splash s4"></span>
              </div>
            </div>
          </div>
          <div class="cup-caption">
            <p class="loading-title">AI &#x6B63;&#x5728;&#x4E3A;&#x4F60;&#x751F;&#x6210;&#x4E2A;&#x6027;&#x5316;&#x5B66;&#x4E60;&#x8DEF;&#x7EBF;</p>
            <p class="loading-subtitle">&#x6B63;&#x5728;&#x8C03;&#x914D;&#x4E13;&#x5C5E;&#x8282;&#x594F;&#xFF0C;&#x8BF7;&#x7A0D;&#x5019;</p>
          </div>
  ...
  </div><!-- 结束 learning-path-container -->
</template>
...
const persistAssessmentState = () => {
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      timelinePhases: timelinePhases.value
    })
  );
};
...
const loadAssessmentState = () => {
  ...
  try {
    ...
    if (parsed.timelinePhases) timelinePhases.value = parsed.timelinePhases;
...
const weekUnit = '\u5468';
const getGanttTitle = (task: any) => `\u7B2C${task.weekNumber}\u5468\u5F00\u59CB\uFF0C\u6301\u7EED${task.duration || 1}\u5468`;

const overviewTitle = computed(() => {
  const summaryTitle = learningPath.value?.summary?.title?.trim();
  const genericTitles = [
    '&#x2715;',
    'AI &#x2715;',
    '&#x2715;?',
  ...
  if (summaryTitle && !genericTitles.includes(summaryTitle)) {
    return summaryTitle as string;
  }
  return '&#x2715;';
});
...
const hasFoundationProgress = () => {
  const phases = learningPath.value?.phases || [];
  return phases.some((phase: any) =>
    (phase.topics || []).some((topic: any) => topic.completed)
  );
};
...
const timelinePhases = ref([
  {
    ...
    topics: [
      { id: 'html-1', name: '标签结构', completed: true },
      { id: 'html-2', name: '语义化', completed: true },
      { id: 'html-3', name: '表单设计', completed: true },
      { id: 'html-4', name: '多媒体', completed: true }
    ],
    progress: 100,
    current: false,
    completed: true
  },
  {
    ...
    topics: [
      { id: 'css-1', name: '选择器', completed: true },
      { id: 'css-2', name: 'Flexbox', completed: true },
      { id: 'css-3', name: 'Grid', completed: true },
      { id: 'css-4', name: '动画', completed: false },
      { id: 'css-5', name: '响应式', completed: false }
    ],
    progress: 60,
    current: true,
    completed: false
  },
  {
    ...
    topics: [
      { id: 'js-1', name: '语法基础', completed: true },
      { id: 'js-2', name: 'DOM操作', completed: false },
      { id: 'js-3', name: '事件处理', completed: false },
      { id: 'js-4', name: 'Ajax', completed: false },
      { id: 'js-5', name: 'ES6+', completed: false }
    ],
    progress: 20,
    current: false,
    completed: false
  },
  {
    ...
    topics: [
      { id: 'vue-1', name: '组件', completed: false },
      { id: 'vue-2', name: '路由', completed: false },
      { id: 'vue-3', name: 'Vuex', completed: false },
      { id: 'vue-4', name: '项目实战', completed: false }
    ],
    progress: 0,
    current: false,
    completed: false
...
// 更新基础学习路径进度
const updateTimelineProgress = (phase: any) => {
  const totalTasks = phase.topics.length;
  const completedTasks = phase.topics.filter((t: any) => t.completed).length;
  phase.progress = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;
  phase.completed = phase.progress === 100;
  
  // 自动设置下一个未完成阶段为current
  const allPhases = timelinePhases.value;
  let foundNext = false;
  allPhases.forEach((p: any) => {
    if (!foundNext && !p.completed) {
      p.current = true;
      foundNext = true;
    } else {
      p.current = false;
...
5. **重要**：monthlyPlan的tasks必须根据用户的时间投入来规划，并保证甘特图不会被截断：
   - 用户每周可投入：${data.weeklyTime}
   - 用户每天可用：${data.dailyTime}
   - 每个task的hours应合理分配，不超过用户每周可用时间
   - **总周期=summary.estimatedWeeks**，必须与用户能坚持的周期（${data.duration}）匹配
   - **所有task的结束周(weekNumber+duration-1)必须<=summary.estimatedWeeks**，避免甘特图被截断
   - monthlyPlan覆盖的最大周数应与summary.estimatedWeeks一致，不得超出
   - 根据用户能坚持的周期（${data.duration}）来设置总的month数量，month数*4周应等于或略小于summary.estimatedWeeks
  ...
  try {
    ...
    if (parsed.timeline && Array.isArray(parsed.timeline)) {
      timelinePhases.value = parsed.timeline.map((phase: any, idx: number) => ({
        ...
        topics: (phase.topics || []).map((t: any, tIdx: number) => ({
          id: t.id || `${phase.id}-topic-${tIdx}`,
          name: t.name || `任务${tIdx + 1}`,
          completed: false
        })),
        progress: 0,
        current: idx === 0,
      ...
      console.log('✅ 已更新timelinePhases:', timelinePhases.value);
    }
...
const updateProgress = () => {
  ...
  learningPath.value.phases.forEach((phase: any) => {
    ...
    phase.progress = topics.length > 0 ? Math.round((completedTopics / topics.length) * 100) : 0;
    phase.completed = topics.length > 0 && completedTopics === topics.length;
    if (!nextCurrentSet && !phase.completed) {
      phase.current = true;
      nextCurrentSet = true;
    } else {
      phase.current = false;
...
const syncMonthlyTaskCompletion = (task: any) => {
  if (!learningPath.value?.phases) return;
  ...
  learningPath.value.phases.forEach((phase: any) => {
    (phase.topics || []).forEach((topic: any) => {
      if (isNameMatch(taskName, topic.name)) {
        topic.completed = !!task.completed;
        hasMatch = true;
      }
    });
  });
...
const syncTopicCompletion = (topic: any) => {
  if (!learningPath.value?.monthlyPlan) return;
  const topicName = topic?.name || '';
  let hasMatch = false;

  learningPath.value.monthlyPlan.forEach((month: any) => {
    (month.tasks || []).forEach((task: any) => {
      if (isNameMatch(topicName, task.name)) {
        task.completed = !!topic.completed;
        hasMatch = true;
      }
    });
  });
...
const handleTopicToggle = (_phase: any, topic: any) => {
  // 先同步到月度计划
...
// 初始化同步：确保月度计划和阶段主题的完成状态一致
const initializeSync = () => {
  if (!learningPath.value?.phases || !learningPath.value?.monthlyPlan) return;
  
  console.log('🔄 开始初始化同步月度计划和学习阶段');
  ...
  learningPath.value.monthlyPlan.forEach((month: any) => {
    (month.tasks || []).forEach((task: any) => {
      if (task.completed) {
        syncMonthlyTaskCompletion(task);
      }
    });
  });
...
const progressStats = computed(() => {
  ...
  const totalPhases = learningPath.value.phases.length;

  learningPath.value.phases.forEach((phase: any) => {
    const topics = phase.topics || [];
    if (topics.length > 0 && topics.every((topic: any) => topic.completed)) {
      completedPhases++;
    }
...
const learningProgress = computed(() => {
  const { totalTopics, completedTopics } = progressStats.value;
...
watchEffect(() => {
  const { totalTopics, completedTopics, completedPhases } = progressStats.value;
  const setUnlocked = (id: string, unlocked: boolean) => {
    const target = achievements.value.find((achievement) => achievement.id === id);
    if (target) target.unlocked = unlocked;
  };

  setUnlocked('ach-1', completedTopics >= 1);
  setUnlocked('ach-2', completedPhases >= 1);
  setUnlocked('ach-3', completedPhases >= 2);
  setUnlocked('ach-4', totalTopics > 0 && completedTopics === totalTopics);
});
</script>

<style scoped>
/* ==================== 艺术字体引入 ==================== */
@import url('https://fonts.googleapis.com/css2?family=Pacifico&family=Poppins:wght@600;700&family=Space+Grotesk:wght@500;600;700&display=swap');
...
.learning-path-container {
  ...
  font-family: 'Space Grotesk', 'Poppins', 'PingFang SC', 'Hiragino Sans GB', 'Microsoft YaHei', sans-serif;
  position: relative;
}
...
.welcome-dots-spinner {
  position: relative;
  width: 150px;
  height: 150px;
  margin: 0 auto 3rem;
  animation: welcomeSpinnerRotate 1.8s linear infinite;
}

.welcome-dots-spinner .dot {
  position: absolute;
  width: 18px;
  height: 18px;
  border-radius: 50%;
  ...
  left: 50%;
  margin: -9px 0 0 -9px;
  transform: rotate(calc(var(--i) * 30deg)) translate(0, -62px);
  background: hsl(338, 70%, calc(78% - (var(--i) * 2%)));
  box-shadow: 0 6px 12px rgba(208, 92, 122, 0.35);
  animation: welcomeDotPulse 1.2s ease-in-out infinite;
...
@keyframes welcomeSpinnerRotate {
  to {
...
@keyframes welcomeDotPulse {
  0%, 100% { 
    opacity: 0.35;
    transform: rotate(calc(var(--i) * 30deg)) translate(0, -62px) scale(0.8);
  }
  50% { 
    opacity: 1;
    transform: rotate(calc(var(--i) * 30deg)) translate(0, -62px) scale(1.2);
  }
}
...
.learning-path-section {
  ...
  --ai-muted: #52606d;
  --ai-surface: #ffffff;
  --ai-soft: rgba(255, 255, 255, 0.78);
  --ai-accent: #ff7a59;
  --ai-accent-strong: #ff5a3c;
  --ai-teal: #1aa6a6;
  --ai-sun: #ffd166;
  --ai-sky: #9bd4ff;
  --ai-lilac: #c9b6ff;
  ...
  border-radius: 28px;
  background:
    radial-gradient(120% 120% at 0% 0%, #fff2e1 0%, #fff7ef 32%, #e7f5ff 68%, #f2fff8 100%),
    repeating-linear-gradient(135deg, rgba(255, 255, 255, 0.2) 0 12px, rgba(255, 255, 255, 0.6) 12px 24px);
...
.learning-path-section::after {
  ...
  bottom: -200px;
  left: -180px;
  background: radial-gradient(circle, rgba(26, 166, 166, 0.25), rgba(26, 166, 166, 0));
  opacity: 0.7;
  pointer-events: none;
}
...
.hero-card {
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.95) 0%, #fff4e6 28%, #e9f4ff 65%, #f4fff7 100%);
  border-radius: 24px;
  padding: 2.25rem;
...
.hero-card::before {
  ...
  top: -70px;
  right: -60px;
  background: radial-gradient(circle, rgba(255, 122, 89, 0.4), rgba(255, 122, 89, 0));
  pointer-events: none;
}
...
.hero-card::after {
  ...
  bottom: -120px;
  left: -80px;
  background: radial-gradient(circle, rgba(26, 166, 166, 0.3), rgba(26, 166, 166, 0));
  pointer-events: none;
}
...
.hero-title {
  ...
  font-weight: 700;
  background: linear-gradient(90deg, #0f172a 0%, #1b6f7a 50%, #0f172a 100%);
...
.learning-path-section .stat-label {
  display: block;
  color: #94a3b8;
  font-size: 0.85rem;
...
.learning-path-section .stat-value {
  font-weight: 700;
  color: var(--ai-ink);
  font-size: 1.2rem;
}
...
.ghost-button:hover {
  transform: translateY(-2px);
  box-shadow: 0 16px 30px rgba(255, 122, 89, 0.45);
...
.path-card.active {
  ...
  box-shadow: 0 18px 36px rgba(255, 122, 89, 0.25);
}
...
.path-card::after {
  ...
  inset: auto 0 0 0;
  height: 6px;
  background: linear-gradient(90deg, rgba(255, 90, 60, 0.35), rgba(26, 166, 166, 0.35));
  opacity: 0.7;
}
...
.tone-rose { background: linear-gradient(135deg, #ffe7dd 0%, #fff3ec 100%); }
.tone-amber { background: linear-gradient(135deg, #fff1cc 0%, #fff8e8 100%); }
.tone-mint { background: linear-gradient(135deg, #e6fbf6 0%, #f2fffb 100%); }
.tone-lilac { background: linear-gradient(135deg, #e8f2ff 0%, #f6f9ff 100%); }
...
.card-icon-indicator.lit {
  color: #b42318;
  background: radial-gradient(circle at 30% 30%, #fff4f0 0%, #ffe3d9 45%, #ffd0bf 100%);
  border-color: rgba(255, 122, 89, 0.6);
  box-shadow:
    0 10px 18px rgba(255, 122, 89, 0.25),
...
.monthly-preview-card {
  background: linear-gradient(135deg, #e8f4ff 0%, #f6fbff 100%);
}
...
.modal-card {
  width: min(920px, 100%);
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.98) 0%, #fff7ef 45%, #f1fbff 100%);
  border-radius: 24px;
...
.monthly-bar-fill {
  height: 10px;
  border-radius: 999px;
  background: linear-gradient(90deg, #ff7a59, #1aa6a6);
}
...
.month-tasks .task-name {
  max-width: 70%;
}

.month-tasks .task-hours {
  color: #0f172a;
  font-weight: 600;
}
...
.loading-overlay {
  position: fixed;
  inset: 0;
  background: radial-gradient(circle at top, #f7bfd1 0%, #f4b2c7 45%, #f6c4d3 100%);
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 1000;
...
.cup-card {
  ...
  box-shadow: 0 24px 60px rgba(81, 34, 44, 0.2);
...
.cup-liquid {
  ...
  bottom: 6px;
  height: 45%;
  background: linear-gradient(180deg, #ff6f9c 0%, #ff3d7f 100%);
  border-radius: 0 0 8px 8px;
  overflow: hidden;
  animation: liquidHue 6s ease-in-out infinite, liquidLevel 3.6s ease-in-out infinite;
}
...
.cup-wave {
  ...
  height: 30px;
  background:
    radial-gradient(circle at 12px 14px, rgba(255, 255, 255, 0.55) 0 12px, transparent 13px) repeat-x,
    linear-gradient(180deg, rgba(255, 255, 255, 0.4), rgba(255, 255, 255, 0));
  background-size: 48px 24px, 100% 100%;
  background-position: 0 6px, 0 0;
  clip-path: inset(0 0 0 0 round 6px);
  animation: waveFlow 2.2s linear infinite, waveBob 2.2s ease-in-out infinite;
  pointer-events: none;
}
...
.splash {
  ...
  border-radius: 50%;
  background: rgba(255, 255, 255, 0.9);
  animation: splashPop 2.8s ease-in-out infinite;
}

.s1 { left: 18px; top: 20px; animation-delay: 0s; }
.s2 { right: 24px; top: 14px; animation-delay: 0.6s; }
.s3 { left: 50px; top: 8px; animation-delay: 1.2s; }
.s4 { right: 58px; top: 26px; animation-delay: 1.8s; }
...
.loading-progress-fill {
  ...
  inset: 0;
  width: 40%;
  background: linear-gradient(90deg, #ff77a8, #ff3d7f);
...
@keyframes waveMove {
  0%, 100% { transform: translateX(-8%) translateY(-18px); }
  50% { transform: translateX(8%) translateY(18px); }
}

@keyframes waveFlow {
  0% { background-position: 0 6px, 0 0; }
  100% { background-position: 96px 6px, 0 0; }
}
...
@keyframes splashPop {
  ...
  30% { opacity: 0.9; }
  60% { transform: translateY(-18px) scale(1); opacity: 0.7; }
...
.timeline-task.completed .timeline-task-label {
  color: #86868b;
...
.step-number {
  ...
  height: 52px;
  border-radius: 10px;
  background: linear-gradient(135deg, #f6b0c4 0%, #f2a1b9 55%, #ec8aa8 100%);
  color: #5b2b3b;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 1.35rem;
  font-weight: 800;
  box-shadow: 0 8px 16px rgba(216, 120, 146, 0.35);
  flex-shrink: 0;
...
.step-content {
  background: linear-gradient(180deg, #fdf7ee 0%, #fbf1e4 100%);
  border-radius: 18px 18px 20px 20px;
  padding: 2.25rem 2rem 2rem;
  box-shadow:
    0 16px 30px rgba(74, 54, 36, 0.18),
    0 4px 12px rgba(74, 54, 36, 0.12);
  border: 1px solid rgba(167, 135, 105, 0.35);
  transition: all 0.3s ease;
  position: relative;
  transform: rotate(-1.2deg);
}

.project-step:nth-child(even) .step-content {
...
.step-content::before {
  ...
  position: absolute;
  inset: 18px;
  border: 2px dashed rgba(197, 160, 120, 0.5);
  border-radius: 14px;
  pointer-events: none;
}
...
.step-content::after {
  ...
  width: 120px;
  height: 36px;
  background: linear-gradient(135deg, rgba(241, 186, 152, 0.95), rgba(228, 150, 118, 0.95));
  border-radius: 6px;
  box-shadow: 0 6px 14px rgba(160, 104, 82, 0.35);
...
.step-content:hover {
  box-shadow:
    0 18px 36px rgba(74, 54, 36, 0.22),
    0 6px 16px rgba(74, 54, 36, 0.16);
...
.icon-cluster .icon-item:nth-child(2) {
  animation-delay: 0.3s;
}

.icon-cluster .icon-item:nth-child(3) {
  animation-delay: 0.6s;
}

.icon-cluster .icon-item:nth-child(4) {
...
.stage-content-box::after {
  content: '';
  position: absolute;
  inset: 14px;
  border-radius: 12px;
  border: 2px dashed rgba(178, 150, 120, 0.4);
  pointer-events: none;
  opacity: 0;
}

.stage-card--0 {
  background: linear-gradient(180deg, #f8f1e6 0%, #f3e7d4 100%);
  border: 1px solid rgba(167, 135, 105, 0.35);
  box-shadow: 0 14px 28px rgba(77, 58, 40, 0.18);
}
...
.stage-card--0::before {
  ...
  width: 120px;
  height: 46px;
  background: linear-gradient(135deg, #e2c4a5, #d8b89a);
  border-radius: 10px;
  opacity: 0.9;
  box-shadow: 0 6px 14px rgba(93, 72, 52, 0.25);
}
...
.stage-card--1 {
  background: linear-gradient(180deg, #f5eee2 0%, #efdfc9 100%);
  border: 1px solid rgba(164, 133, 104, 0.3);
  box-shadow: 0 12px 24px rgba(82, 62, 44, 0.16);
}
...
.stage-card--1::before {
  ...
  width: 120px;
  height: 50px;
  background: linear-gradient(135deg, rgba(187, 166, 142, 0.9), rgba(165, 144, 121, 0.9));
  transform: rotate(-8deg);
  border-radius: 10px;
  box-shadow: 0 6px 16px rgba(88, 68, 50, 0.3);
}
...
.stage-card--2 {
  background: linear-gradient(180deg, #efe6d7 0%, #e6d4bd 100%);
  border: 1px solid rgba(139, 117, 94, 0.3);
  box-shadow: 0 16px 28px rgba(74, 57, 42, 0.2);
}
...
.stage-card--2::before {
  ...
  width: 70px;
  height: 70px;
  background: radial-gradient(circle at center, rgba(237, 217, 196, 0.9), rgba(186, 163, 137, 0.9));
  border-radius: 50%;
  box-shadow: 0 6px 16px rgba(88, 68, 50, 0.3);
}
...
.stage-card--3 {
  background: linear-gradient(180deg, #f7efe3 0%, #f1e2cd 100%);
  border: 1px solid rgba(160, 134, 105, 0.3);
  box-shadow: 0 14px 30px rgba(78, 58, 40, 0.2);
}
...
.stage-card--3::before {
  ...
  width: 140px;
  height: 50px;
  background: linear-gradient(135deg, rgba(197, 176, 151, 0.9), rgba(170, 147, 120, 0.9));
  border-radius: 12px;
  box-shadow: 0 6px 16px rgba(88, 68, 50, 0.28);
}
...
.summary-card {
  background: linear-gradient(135deg, #ff7a59 0%, #ffb86b 45%, #4fb6b2 100%);
  border-radius: 24px;
  padding: 2.5rem;
  box-shadow: 0 16px 50px rgba(255, 122, 89, 0.35);
  color: white;
  position: relative;
  overflow: hidden;
}
